<?php
	...
	class libraryTwitter extends socialxeLibrary
	{
		...
		function authenticate()
		{
			...
			if(!$oauth_verifier || !$_SESSION['twitter_oauth_token_secret'] || !$_SESSION['twitter_oauth_token'] || $_SESSION['twitter_oauth_token'] !== $oauth_token) return new BaseObject(-1, "msg_invalid_request");
		...
		function setProfile($profile_info = null)
		{
			...
			if(!$profile) return new BaseObject(-1, "msg_errer_api_connect");
			...
			if($this->config->sns_suspended_account == 'Y'){
				...
				if(!$user['id']) return new BaseObject(-1, 'msg_sns_suspended_account');
			}
