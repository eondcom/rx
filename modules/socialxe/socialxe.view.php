<?php
	...
	class socialxeView extends socialxe
	{
		...
		{
			if(!$_SESSION['tmp_socialxe_confirm_email']) return new BaseObject(-1, "msg_invalid_request");
		...
		{
			if(!$_SESSION['tmp_socialxe_input_add_info']) return new BaseObject(-1, "msg_invalid_request");
		...
		{
			if(isCrawler()) return new BaseObject(-1, "msg_invalid_request");
			
			$service = Context::get('service');	
			if(!$service || !in_array($service,$this->config->sns_services)) return new BaseObject(-1, "msg_not_support_service_login");
			...
			if($type == 'register'){
				if(!$is_logged) return new BaseObject(-1, "msg_not_logged");
			}elseif($type == 'login'){
				if($is_logged) return new BaseObject(-1, "already_logged");
			}
		...
		{
			if($this->config->sns_profile != 'Y') return new BaseObject(-1, "msg_invalid_request");
			...
			if(!$member_info->member_srl) return new BaseObject(-1, "msg_invalid_request");
